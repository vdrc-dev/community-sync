...
import { Shield, Brain, MessageSquare, CheckCircle2, TrendingUp } from 'lucide-react';
          ...
          <div className="ml-5 pl-1 mt-2 flex items-center gap-3">
            <div className="flex-1 max-w-xs h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
              <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, hsl(185 70% 50%), hsl(280 70% 60%), hsl(330 70% 60%))' }}
                {...(isExporting ? { style: { width: '66%', background: 'linear-gradient(90deg, hsl(185 70% 50%), hsl(280 70% 60%), hsl(330 70% 60%))' } } : { initial: { width: '0%' }, animate: { width: '66%' }, transition: { delay: 0.3, duration: 1.2, ease: 'easeOut' } })} />
            </div>
            <span className="text-xs font-bold text-white/30">Semana 3 de 4</span>
          </div>
        </motion.div>
          ...
          {WEEKS.map((week, i) => {
            ...
            return (
              <motion.button key={week.num} {...m(0.15 + i * 0.1)}
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-0.5 rounded-b-full" style={{ background: week.color, opacity: isActive ? 0.8 : 0.2 }} />
                {isCurrent && (
                  <motion.div className="absolute top-3 right-3 px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wider"
                ...
                <div className="mb-3 px-3 py-1.5 rounded-lg border border-white/[0.06] bg-white/[0.02] inline-flex items-center gap-2">
                  ...
                  <span className="text-xs font-black" style={{ color: week.color }}>{week.metric.value}</span>
                </div>
                  ...
                  {isActive && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-1.5">
                      ...
                      <div className="mt-2 p-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                        <p className="text-[10px] text-white/30 italic">"{week.impact}"</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
