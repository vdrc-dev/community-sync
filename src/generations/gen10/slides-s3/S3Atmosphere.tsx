      ...
      {showAurora && !isExporting && (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <motion.div
            ...
            style={{
              background: `linear-gradient(90deg, transparent 5%, hsl(${primaryHue} 75% 50% / ${0.04 * opMul}) 20%, hsl(${secondaryHue} 60% 50% / ${0.06 * opMul}) 40%, hsl(${tertiaryHue} 70% 55% / ${0.04 * opMul}) 60%, hsl(${primaryHue} 65% 45% / ${0.03 * opMul}) 80%, transparent 95%)`,
              filter: 'blur(60px)',
              borderRadius: '50%',
            }}
          ...
          <motion.div
            ...
            style={{
              background: `linear-gradient(90deg, transparent 10%, hsl(${secondaryHue} 70% 55% / ${0.03 * opMul}) 30%, hsl(${primaryHue} 60% 50% / ${0.05 * opMul}) 50%, hsl(${tertiaryHue} 65% 50% / ${0.03 * opMul}) 70%, transparent 90%)`,
              filter: 'blur(50px)',
              borderRadius: '50%',
            }}
          ...
          {[0, 1, 2].map((i) => (
            <motion.div
              ...
              style={{
                ...
                width: '2px',
                height: '120%',
                background: `linear-gradient(180deg, hsl(${[primaryHue, secondaryHue, tertiaryHue][i]} 60% 60% / ${0.08 * opMul}), transparent 80%)`,
                filter: 'blur(8px)',
      ...
      {showOrbs && !isExporting && (
        <>
          <motion.div
            ...
            style={{ background: `hsl(${primaryHue} 70% 45% / ${0.12 * opMul})` }}
            animate={{ scale: [1, 1.25, 1], opacity: [0.1, 0.24, 0.1], rotate: [0, 5, 0] }}
            transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute bottom-[-10%] right-[5%] w-[650px] h-[650px] rounded-full blur-[220px]"
            style={{ background: `hsl(${secondaryHue} 55% 48% / ${0.08 * opMul})` }}
            animate={{ scale: [1.1, 0.92, 1.1], opacity: [0.06, 0.18, 0.06], rotate: [0, -3, 0] }}
            transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut', delay: 4 }}
          />
          <motion.div
            className="absolute top-[35%] right-[20%] w-[400px] h-[400px] rounded-full blur-[180px]"
            style={{ background: `hsl(${tertiaryHue} 55% 48% / ${0.07 * opMul})` }}
            animate={{ x: [0, 30, 0], y: [0, -20, 0], opacity: [0.04, 0.14, 0.04] }}
            transition={{ duration: 16, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
          />
          <motion.div
            className="absolute top-[8%] right-[28%] w-[560px] h-[320px] rounded-full blur-[190px]"
            style={{ background: `hsl(${secondaryHue} 72% 54% / ${0.1 * opMul})` }}
