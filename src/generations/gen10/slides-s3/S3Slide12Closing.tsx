...
const MISSIONS = [
  { task: 'Dashboard con datos reales', detail: 'Sube tu CSV a Canvas → exporta HTML', icon: BarChart3, accent: S3_ACCENT.cyan, difficulty: 'Fácil', time: '~15 min' },
  { task: 'Claude Desktop + 2 Skills', detail: 'Instala PDF Skill + PPTX Skill', icon: Terminal, accent: S3_ACCENT.violet, difficulty: 'Medio', time: '~20 min' },
  { task: 'MCP en acción o Video AI', detail: 'Conecta tu CRM vía MCP o genera un clip en Krea', icon: Zap, accent: S3_ACCENT.amber, difficulty: 'Desafío', time: '~30 min' },
];
  ...
  return (
    <div className={S3_ROOT_CLASS + ' flex flex-col items-center justify-center ' + S3_CONTENT_PADDING} style={{ background: S3_THEME.background }}>
      <div className="absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_30%_25%,_hsl(150_60%_50%_/_0.12),_transparent_65%)]" />
        ...
        <div className="flex flex-col sm:flex-row items-start justify-between mb-6 gap-4">
          <div>
            <motion.div {...me(0)} className="flex items-center gap-3 mb-3">
              <motion.div className="w-1.5 h-12 rounded-full"
                style={{ background: `linear-gradient(180deg, ${S3_ACCENT.emerald.text}, ${S3_ACCENT.cyan.text})` }}
          ...
          <motion.div {...me(0.08)} className="flex items-center gap-3 flex-wrap">
            <motion.div
              ...
              {...(isExporting ? {} : {
                whileHover: { scale: 1.06, boxShadow: '0 0 40px hsl(160 65% 50% / 0.25)' },
                animate: { borderColor: ['hsl(330 70% 60% / 0.25)', 'hsl(160 65% 50% / 0.35)', 'hsl(330 70% 60% / 0.25)'] },
            ...
            {WEEK_STATS.map((s, i) => (
              <motion.div key={i}
                ...
                className="text-center px-4 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.02] relative overflow-hidden"
              >
                {!isExporting && (
                  <motion.div className="absolute inset-0 pointer-events-none"
                ...
                <motion.p className="text-2xl font-black relative"
                  style={{
                    background: `linear-gradient(135deg, hsl(${s.hue} 65% 60%), hsl(${s.hue + 30} 60% 55%))`,
                    WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
                  }}
                  {...(isExporting ? {} : { animate: { scale: [1, 1.1, 1] }, transition: { delay: 1 + i * 0.2, duration: 0.6, type: 'spring' } })}
                >{s.num}</motion.p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 sm:gap-6">
          ...
          <motion.div {...me(0.15)} className="col-span-1 sm:col-span-2 rounded-2xl border border-white/[0.08] bg-white/[0.015] p-5 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-4 relative">
              ...
              <motion.span className="ml-auto text-[10px] font-black px-2 py-0.5 rounded-full"
                style={{ background: S3_ACCENT.emerald.bg, color: S3_ACCENT.emerald.text, border: `1px solid ${S3_ACCENT.emerald.border}` }}
                {...(isExporting ? {} : { animate: { scale: [1, 1.08, 1] }, transition: { duration: 2, repeat: Infinity } })}
              >10 tools</motion.span>
            </div>
                ...
                return (
                  <motion.div key={i}
                    {...(isExporting ? {} : {
                      ...
                      transition: { delay: 0.25 + i * 0.05, type: 'spring', stiffness: 180 },
                    })}
          ...
          <div className="col-span-3 flex flex-col gap-4">
            <motion.div {...me(0.2)} className="flex-1 rounded-2xl border p-5 relative overflow-hidden"
              ...
              <div className="flex items-center gap-2 mb-4 relative">
                ...
                <motion.span className="ml-auto flex items-center gap-1.5 text-[10px] text-white/40 font-medium px-3 py-1 rounded-full border"
                  style={{ borderColor: S3_ACCENT.emerald.border, background: S3_ACCENT.emerald.bg }}
                  {...(isExporting ? {} : { animate: { boxShadow: ['0 0 0px hsl(160 65% 45% / 0)', '0 0 20px hsl(160 65% 45% / 0.15)', '0 0 0px hsl(160 65% 45% / 0)'] }, transition: { duration: 2.5, repeat: Infinity } })}
                >
                  <Target className="w-3 h-3" style={{ color: S3_ACCENT.emerald.text }} />
                  <span style={{ color: S3_ACCENT.emerald.text }}>Elige al menos 1</span>
                </motion.span>
              </div>
                ...
                {MISSIONS.map((mission, i) => (
                  <motion.div key={i}
                    {...(isExporting ? {} : {
                      ...
                      transition: { delay: 0.4 + i * 0.12, duration: 0.6, ease: S3_EASE },
                    })}
                  ...
                  >
                    ...
                    <motion.div className="relative w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: `${mission.accent.text}15`, border: `1px solid ${mission.accent.text}25` }}
                      {...(isExporting ? {} : { whileHover: { scale: 1.15, rotate: 5 } })}
                    >
                    ...
                    <div className="relative flex items-center gap-2 shrink-0">
                      ...
                      {mission.difficulty === 'Desafío' && (
                        <motion.div {...(isExporting ? {} : { animate: { rotate: [0, 15, -15, 0] }, transition: { duration: 2, repeat: Infinity } })}>
                          <Star className="w-3.5 h-3.5" style={{ color: S3_ACCENT.amber.text }} />
                        </motion.div>
                      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <motion.div {...me(0.45)}
                ...
                style={{ borderColor: S3_ACCENT.cyan.border, background: S3_ACCENT.cyan.bg, boxShadow: isExporting ? undefined : `0 0 30px ${S3_ACCENT.cyan.glow}` }}
              >
                {!isExporting && (
                  <>
                    <motion.div className="absolute inset-0 pointer-events-none"
                    ...
                    <motion.div className="absolute -inset-4 rounded-2xl pointer-events-none"
                      style={{ background: `radial-gradient(circle at center, ${S3_ACCENT.cyan.glow}, transparent 70%)` }}
                ...
                <motion.div className="relative z-10" {...(isExporting ? {} : { animate: { x: [0, 8, 0] }, transition: { duration: 1.2, repeat: Infinity, ease: S3_EASE } })}>
              ...
              {/* Key takeaway */}
              <motion.div {...me(0.5)}
                className="relative overflow-hidden flex items-start gap-3 px-5 py-4 rounded-xl border border-white/[0.08] bg-white/[0.02] group"
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-xs text-white/60 leading-relaxed">
                    ...
                    Domina <motion.span style={{ color: S3_ACCENT.emerald.text }} className="font-bold inline-block"
                      {...(isExporting ? {} : { animate: { scale: [1, 1.15, 1] }, transition: { duration: 2, repeat: Infinity } })}
                    >2-3</motion.span> que resuelvan <span className="text-white/80 font-semibold">tu</span> problema real.
                  </p>
                </div>
              </motion.div>
            </div>
