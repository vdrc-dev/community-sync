...
import { Rocket, BarChart3, Terminal, Database, Code2, BookOpen, Video, Presentation, ArrowRight, Palette, Zap, Sparkles, Star, TrendingUp, Trophy } from 'lucide-react';
  ...
  return (
    <div className={S3_ROOT_CLASS + ' flex flex-col items-center justify-center ' + S3_CONTENT_PADDING} style={{ background: S3_THEME.background }}>
      ...
      <div className="absolute inset-0">
        <img src={bgClosing} alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.14]" />
      ...
      {!isExporting && (
        <>
          <motion.div
            ...
            style={{ background: 'conic-gradient(from 180deg at 0% 0%, hsl(160 65% 50% / 0.1), transparent 25%)' }}
            animate={{ opacity: [0.4, 0.9, 0.4] }}
            transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute bottom-0 right-0 w-96 h-96 pointer-events-none"
            style={{ background: 'conic-gradient(from 0deg at 100% 100%, hsl(263 60% 55% / 0.08), transparent 25%)' }}
          ...
          <div>
            <motion.div {...me(0)} className="flex items-center gap-3 mb-3">
              <div className="relative">
                ...
                {!isExporting && (
                  <motion.div
                    className="absolute inset-0 w-1.5 rounded-full blur-sm"
              ...
              {!isExporting && (
                <>
                  <motion.div
                    ...
                    style={{ background: 'hsl(160 65% 50% / 0.12)' }}
                    animate={{ opacity: [0.08, 0.25, 0.08] }}
                    transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                  />
                  <motion.div className="absolute inset-0 pointer-events-none"
                    style={{ background: 'linear-gradient(105deg, transparent 35%, hsl(160 65% 60% / 0.12) 50%, transparent 65%)' }}
                    animate={{ x: ['-150%', '250%'] }}
        ...
        <div className="grid grid-cols-5 gap-6">
            ...
            <div className="grid grid-cols-2 gap-2 relative">
              {TOOLS_LEARNED.map((tool, i) => {
                ...
                return (
                  ...
                  >
                    {!isExporting && (
                      <div className="absolute -inset-px rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-lg pointer-events-none"
          ...
          <div className="col-span-3 flex flex-col gap-4">
            ...
            <motion.div {...me(0.2)} className="flex-1 rounded-2xl border p-5 relative overflow-hidden"
              ...
              {!isExporting && (
                <motion.div
                  className="absolute -inset-4 blur-[80px] pointer-events-none rounded-full"
                  style={{ background: 'hsl(160 50% 40% / 0.06)' }}
                ...
                {MISSIONS.map((mission, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Hover glow */}
                      {!isExporting && (
                        <div className="absolute -inset-px rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-400 blur-lg pointer-events-none"
                          style={{ background: mission.accent.glow }} />
                      )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: isExporting ? undefined : `0 0 50px ${S3_ACCENT.cyan.glow}, 0 0 80px hsl(185 70% 50% / 0.12)`,
                }}
              >
                {!isExporting && (
                  <>
                    ...
                    <motion.div
                      className="absolute inset-0 pointer-events-none"
                      style={{ background: 'linear-gradient(105deg, transparent 35%, hsl(185 60% 70% / 0.1) 50%, transparent 65%)' }}
                      animate={{ x: ['-150%', '250%'] }}
                      transition={{ duration: 3, repeat: Infinity, ease: 'linear', repeatDelay: 4 }}
                    />
                  </>
              ...
              <motion.div {...me(0.5)} className="relative overflow-hidden flex items-start gap-3 px-5 py-4 rounded-xl border border-white/[0.08] bg-white/[0.02] group"
                ...
                <div className="relative mt-0.5">
                  <TrendingUp className="w-5 h-5 text-emerald-400/60 shrink-0" />
                  {!isExporting && (
                    <motion.div
                      className="absolute -inset-2 rounded-full blur-md pointer-events-none"
                      style={{ background: 'hsl(160 65% 50% / 0.15)' }}
