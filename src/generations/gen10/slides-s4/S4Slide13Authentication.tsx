      ...
      <div className="relative z-10 max-w-[1400px] mx-auto w-full">
        ...
        <motion.div {...m(0)} className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <span className="text-xs font-black tracking-[0.25em] uppercase text-white/30">Aplicación · Seguridad</span>
        ...
        <div className="grid grid-cols-5 gap-5">
                ...
                <motion.div key={i} {...m(0.1 + i * 0.08)}
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-black tracking-widest uppercase" style={{ color: `hsl(${layer.hue} 55% 60% / 0.7)` }}>CAPA {layer.num}</span>
                    ...
                    <p className="text-[11px] text-white/35 mt-0.5">{layer.desc}</p>
                  </div>
                </motion.div>
              );
            })}
          ...
          <motion.div {...m(0.3)} className="col-span-3 rounded-2xl border border-rose-500/18 overflow-hidden"
            ...
            <div className="p-6">
              ...
              {/* Email input */}
              <div className="mb-3">
                <label className="text-[10px] font-black text-white/25 uppercase tracking-widest">Email corporativo</label>
                <div className={`mt-2 p-3.5 rounded-xl border bg-black/40 font-mono text-sm transition-all duration-500 ${simState === 'denied' ? 'border-red-500/35 shadow-[0_0_15px_hsl(0_60%_40%_/_0.15)]' : (simState === 'approved' || simState === 'rls') ? 'border-emerald-500/35 shadow-[0_0_15px_hsl(150_60%_40%_/_0.15)]' : 'border-white/[0.07]'}`}>
                  <span className={`text-sm ${emailInput ? 'text-white/60' : 'text-white/15'}`}>{emailInput || 'usuario@dominio.cl'}</span>
              ...
              {/* Password */}
              <div className="mb-5">
                <label className="text-[10px] font-black text-white/25 uppercase tracking-widest">Contraseña</label>
                ...
                {simState === 'denied' && (
                  <motion.div key="denied" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-red-400/50">@gmail.com no es un dominio autorizado</p>
                    </div>
                  </motion.div>
                )}
                {(simState === 'approved' || simState === 'rls') && (
                  <motion.div key="ok" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-emerald-400/50">Dominio @fundaciontrabun.cl verificado</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                ...
                {simState === 'rls' && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4 overflow-hidden">
                    <div className="p-4 rounded-xl border border-emerald-500/15 bg-emerald-500/[0.04]">
                      <p className="text-[10px] font-black text-emerald-400/70 uppercase tracking-widest mb-3">Row Level Security — María solo ve sus datos</p>
                      <div className="space-y-2 font-mono text-[10px]">
                        <div className="flex items-center gap-2 p-2.5 rounded-lg bg-emerald-500/[0.06] border border-emerald-500/15">
                          ...
                          <span className="text-white/55">María López · Colegio San Pedro · <span className="text-emerald-400 font-bold">visible ✓</span></span>
                        </div>
                        <div className="flex items-center gap-2 p-2.5 rounded-lg bg-white/[0.01] border border-white/[0.04] opacity-35">
                          ...
                          <span className="text-white/25">Pedro Sánchez · Colegio Norte · <span className="text-red-400/50 font-bold">oculto 🔒</span></span>
                        </div>
                        <div className="flex items-center gap-2 p-2.5 rounded-lg bg-white/[0.01] border border-white/[0.04] opacity-35">
                          <Lock className="w-3.5 h-3.5 text-white/25 shrink-0" />
                          <span className="text-white/25">Ana García · Colegio Sur · <span className="text-red-400/50 font-bold">oculto 🔒</span></span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
