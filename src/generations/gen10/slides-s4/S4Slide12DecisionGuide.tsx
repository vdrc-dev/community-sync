  ...
  return (
    <div className="h-full w-full min-h-screen relative overflow-hidden flex flex-col justify-center px-16 2xl:px-20 font-sans" style={{ background: '#04030a' }}>
      ...
      <S4TeachingRibbon
        ...
        hue={38}
        objective="Tomar decisiones rapidas y correctas sobre que herramienta usar en cada etapa."
        deliverable="Arbol de decision operativo listo para aplicar en el siguiente proyecto."
        qualityGate="Elegir una herramienta debe reducir tiempo y aumentar calidad, no solo verse moderno."
      />
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto w-full">
        ...
        <motion.div {...m(0)} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <span className="text-xs font-black tracking-[0.25em] uppercase text-white/80">Aplicación · Toma de Decisiones</span>
        ...
        <div className="grid grid-cols-2 gap-6">
            ...
            {DECISIONS.map((d, i) => {
              ...
              return (
                <motion.button key={i} {...m(0.12 + i * 0.07)}
                  ...
                  <HelpCircle className="h-5 w-5 shrink-0 text-white/85" />
                  <span className="text-sm font-bold text-white/75 flex-1">{d.q}</span>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  ...
                  <div className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.025]">
                    <div className="flex items-start gap-2.5">
                      ...
                      <p className="text-xs text-white/70 leading-relaxed">{DECISIONS[active].tip}</p>
                    </div>
                  </div>
                </motion.div>
              )}
          ...
          <motion.div {...m(0.45)} className="relative p-6 rounded-2xl border border-amber-500/20 bg-amber-500/[0.04] overflow-hidden"
              ...
              {GOLDEN_RULES.map((rule, i) => (
                <motion.div key={i}
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-white/80 leading-relaxed">{rule.detail}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-4 p-3.5 rounded-xl bg-black/25 border border-white/[0.05]">
              <p className="text-[11px] text-white/80 italic leading-relaxed">"Lo importante no es qué herramienta. Es que empieces. La primera app es la más difícil."</p>
              <p className="mt-1 text-[11px] text-white/85">— Vicente Donoso R.</p>
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <S4Footer
        ...
        contextHint="Guia de decision por escenario"
        hue={38}
        session="S4"
      />
    </div>
