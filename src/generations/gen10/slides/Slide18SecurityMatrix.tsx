...
const COLUMNS = [
  { title: 'PROBLEMA', color: 'rose' as const, items: [{ icon: Lock, label: 'Acceso Vulnerable' }, { icon: KeyRound, label: 'Contraseñas Repetidas' }, { icon: HelpCircle, label: 'Olvido y Frustración' }] },
  { title: 'SOLUCIÓN', color: 'amber' as const, items: [{ icon: Cog, label: 'Generador Complejo' }, { icon: Vault, label: 'Almacenamiento Seguro' }, { icon: Cloud, label: 'Sincronización en la Nube' }] },
  { title: 'BENEFICIO', color: 'emerald' as const, items: [{ icon: Shield, label: 'Seguridad Robusta' }, { icon: Flame, label: 'Protección Contra Amenazas' }, { icon: Smile, label: 'Acceso Fluido y Rápido' }] },
];
const COLOR_MAP = {
    ...
    <S1Shell footerLabel="HIGIENE DIGITAL" className="flex flex-col" radials={
      ...
      <div className="relative z-10 flex flex-col h-full px-5 sm:px-12 py-8">
        <motion.header {...m(0.1)} className="mb-6">
          <div className="flex items-center gap-2.5 px-4 py-2 bg-rose-500/10 border border-rose-500/30 rounded-full backdrop-blur-sm w-fit mb-3">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight">
            Problema → Solución → <span className="text-emerald-400">Beneficio</span>
          </h1>
        </motion.header>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {myth && (
                <div className="bg-rose-500/[0.06] border border-rose-500/30 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2"><AlertTriangle className="w-5 h-5 text-rose-400" /><span className="text-rose-400 font-bold text-sm uppercase">Mito</span></div>
              ...
              {reality && (
                <div className="bg-emerald-500/[0.06] border border-emerald-500/30 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2"><CheckCircle className="w-5 h-5 text-emerald-400" /><span className="text-emerald-400 font-bold text-sm uppercase">Realidad</span></div>
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-5 mb-4">
          {COLUMNS.map((column, colIdx) => {
            ...
            return (
              <motion.div key={column.title} {...m(0.15 + colIdx * 0.1)} className="flex flex-col">
                ...
                <div className={`flex-1 bg-white/[0.02] rounded-b-2xl p-4 border ${colors.border} border-t-0 space-y-3`}>
                  {column.items.map((item, i) => (
                    <motion.div key={i} {...m(0.25 + colIdx * 0.1 + i * 0.05)} className={`flex items-center gap-3 p-3 ${colors.bg} rounded-xl border ${colors.border} shadow-lg ${colors.glow}`}>
          ...
          <div className="bg-white/[0.03] backdrop-blur-sm rounded-2xl p-5 border border-white/10">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-10">
              <div className="flex items-center gap-3 px-4 py-2 bg-blue-600/10 border border-blue-600/30 rounded-xl">
                <div className="w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center"><Shield className="w-5 h-5 text-white" /></div>
                <span className="text-white font-bold">Bitwarden</span>
              </div>
              <div className="flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-xl">
                <svg viewBox="0 0 24 24" className="w-8 h-8"><path fill="white" d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82C19.47,8.88 17.39,10.1 17.41,12.63C17.44,15.65 20.06,16.66 20.09,16.67C20.06,16.74 19.67,18.11 18.71,19.5M13,3.5C13.73,2.67 14.94,2.04 15.94,2C16.07,3.17 15.6,4.35 14.9,5.19C14.21,6.04 13.07,6.7 11.95,6.61C11.8,5.46 12.36,4.26 13,3.5Z" /></svg>
              ...
              <div className="flex items-center gap-3 px-4 py-2 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                <div className="w-10 h-10 rounded-lg bg-blue-500 flex items-center justify-center"><Lock className="w-5 h-5 text-white" /></div>
