...
export function Slide27RoadmapVertical() {
  ...
  const sessions = [
    {
      ...
      desc: 'Inbox Cero, Bitwarden y navegación limpia.',
      icon: Shield,
      current: true,
    ...
    },
    {
      ...
      desc: 'Claude, Perplexity y Canvas.',
      icon: BarChart3,
      current: false,
      color: 'cyan' as const,
    },
    ...
    },
    {
      num: 4,
      title: 'VIBE CODING',
      desc: 'Lovable, Supabase y Cursor.',
      icon: Rocket,
      current: false,
  ...
  const COLOR_MAP = {
    emerald: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-400', accent: 'bg-emerald-500', dot: 'bg-emerald-400' },
    cyan: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/30', text: 'text-cyan-400', accent: 'bg-cyan-500', dot: 'bg-white/30' },
    violet: { bg: 'bg-violet-500/10', border: 'border-violet-500/30', text: 'text-violet-400', accent: 'bg-violet-500', dot: 'bg-white/30' },
    amber: { bg: 'bg-amber-500/10', border: 'border-amber-500/30', text: 'text-amber-400', accent: 'bg-amber-500', dot: 'bg-white/30' },
  } as const;
  ...
  return (
    <div className="h-full w-full min-h-screen relative overflow-hidden bg-[#020609] flex flex-col font-sans selection:bg-emerald-500/30">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-t from-[#020609] via-[#020609]/80 to-[#020609]/60" />
      ...
      <div className="relative z-10 flex flex-col h-full px-12 py-8">
        ...
        <motion.div {...getMotionProps(0.1)} className="mb-6">
          ...
          <p className="text-white/50 text-lg mt-2">
            Estás aquí: construyendo los cimientos
          </p>
        </motion.div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="relative w-full max-w-6xl">
            {/* Horizontal line */}
            <div className="absolute top-[52px] left-0 right-0 h-1 bg-gradient-to-r from-emerald-500 via-cyan-500/50 via-violet-500/30 to-amber-500/20 rounded-full" />
            ...
            <div className="grid grid-cols-4 gap-5">
              {sessions.map((session, i) => {
                ...
                return (
                  ...
                  >
                    {/* Timeline dot */}
                    <div className={`relative z-10 mb-4 ${session.current ? 'scale-110' : ''}`}>
                    ...
                    {/* Content card */}
                    <div className={`w-full p-5 rounded-2xl ${colors.bg} border ${colors.border} backdrop-blur-sm ${session.current ? 'shadow-xl shadow-emerald-500/10' : ''}`}>
                      {/* Current badge */}
                      {session.current && (
                        <div className="flex items-center gap-2 mb-3">
                          <CheckCircle className="w-4 h-4 text-emerald-400" />
                          <span className="text-emerald-400 text-xs font-bold uppercase tracking-wider">HOY</span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <div className={`w-11 h-11 rounded-xl ${colors.bg} border ${colors.border} flex items-center justify-center`}>
                          <session.icon className={`w-5 h-5 ${colors.text}`} strokeWidth={1.5} />
                        </div>
                      ...
                      {/* Title */}
                      <h3 className={`font-bold ${session.current ? 'text-white' : 'text-white/80'} text-sm mb-2 leading-tight`}>
