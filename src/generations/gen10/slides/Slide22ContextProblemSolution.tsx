      ...
      <div className="relative z-10 flex flex-col h-full px-12 py-8">
        ...
        <motion.header {...getMotionProps(0.1)} className="mb-6">
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-2">
            MÁS IMPORTANTE QUE EL PROMPTING:
          </h1>
          <h2 
        ...
        <div className="flex-1 grid grid-cols-3 gap-5 mb-4">
          ...
          {/* Column 1: El Problema */}
          <motion.div {...getMotionProps(0.2)} className="flex flex-col">
            <div className="bg-rose-500/10 rounded-t-2xl p-4 border border-rose-500/30 border-b-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-black text-rose-400 tracking-wider">EL PROBLEMA</h3>
              </div>
            </div>
            <div className="flex-1 bg-white/[0.02] rounded-b-2xl p-5 border border-rose-500/30 border-t-0">
              ...
              <p className="text-white/70 text-sm leading-relaxed">
                Los modelos vienen "en blanco", tienen <span className="text-rose-400 font-semibold">amnesia entre chats</span> y asumen por defecto un formato estándar (generalmente con sesgo de EE.UU.).
              </p>
              <div className="mt-5 flex justify-center">
                <div className="relative">
                  <Brain className="w-16 h-16 text-rose-400/40" strokeWidth={1} />
                  <span className="absolute -top-1 -left-1 text-rose-400 text-xl font-bold">?</span>
                  <span className="absolute -bottom-1 -right-1 text-rose-400 text-lg font-bold">?</span>
                </div>
              </div>
            </div>
          </motion.div>

          {/* Column 2: La Solución */}
          <motion.div {...getMotionProps(0.3)} className="flex flex-col">
            <div className="bg-violet-500/10 rounded-t-2xl p-4 border border-violet-500/30 border-b-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-black text-violet-400 tracking-wider">LA SOLUCIÓN</h3>
              </div>
            </div>
            <div className="flex-1 bg-white/[0.02] rounded-b-2xl p-5 border border-violet-500/30 border-t-0">
              ...
              <p className="text-white/70 text-sm leading-relaxed">
                Cargar tu <span className="text-violet-400 font-semibold">"memoria estática"</span> (Manuales, PDFs, Guías) en el cerebro de la IA antes de hacer la primera pregunta.
              </p>
          ...
          {/* Column 3: Aplicación */}
          <motion.div {...getMotionProps(0.4)} className="flex flex-col">
            <div className="bg-emerald-500/10 rounded-t-2xl p-4 border border-emerald-500/30 border-b-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-black text-emerald-400 tracking-wider">APLICACIÓN</h3>
              </div>
            </div>
            <div className="flex-1 bg-white/[0.02] rounded-b-2xl p-5 border border-emerald-500/30 border-t-0">
              ...
              <div className="space-y-3">
                {platforms.map((platform, i) => (
                  <motion.div
                    key={platform.name}
                    {...getMotionProps(0.5 + i * 0.08)}
                    className="flex items-center gap-3 p-2 bg-white/[0.03] rounded-lg border border-white/5"
                  >
                    <div className={`w-9 h-9 rounded-lg ${platform.color} flex items-center justify-center`}>
                      <span className="text-white font-bold text-xs">{platform.label}</span>
                    </div>
                    <div>
                      <p className="text-white font-semibold text-sm">{platform.name}</p>
                      <p className="text-white/50 text-xs">{platform.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
