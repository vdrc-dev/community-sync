...
export function Slide20BitwardenInstall() {
  ...
  return (
    <div className="h-full w-full min-h-screen relative overflow-hidden bg-[#020609] flex flex-col font-sans selection:bg-blue-500/30">
      ...
      <div className="relative z-10 flex flex-col h-full px-12 py-8">
        ...
        <motion.header {...getMotionProps(0.1)} className="mb-6">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center gap-2.5 px-4 py-2 bg-blue-600/15 border border-blue-600/30 rounded-full backdrop-blur-sm">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
            Instalación de{' '}
            <span className="text-blue-400">Bitwarden</span>
          </h1>
        </motion.header>
        ...
        <div className="flex-1 flex gap-8">
          ...
          {/* Left Column - Video */}
          <motion.div {...getMotionProps(0.2)} className="flex-1 flex flex-col">
            <div className="flex items-center gap-3 mb-3">
            ...
            <div className="relative flex-1">
              ...
              <div className="relative h-full p-[2px] rounded-xl bg-gradient-to-br from-blue-600/60 to-cyan-500/40">
                <div className="relative h-full rounded-lg overflow-hidden bg-[#020609]">
                  <iframe
                    src={`https://www.youtube.com/embed/${YOUTUBE_VIDEO_ID}?rel=0&modestbranding=1`}
                    title="Tutorial de Bitwarden para Principiantes"
            ...
            <motion.div {...getMotionProps(0.5)} className="mt-4 flex justify-center">
              <a 
                href={`https://www.youtube.com/watch?v=${YOUTUBE_VIDEO_ID}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-4 py-2 bg-blue-600/10 border border-blue-600/30 rounded-lg text-blue-400 hover:bg-blue-600/20 transition-colors text-sm"
              >
                <ExternalLink className="w-4 h-4" />
                <span className="font-medium">Ver en YouTube</span>
              </a>
            </motion.div>
          </motion.div>

          {/* Right Column - Steps */}
          <motion.div {...getMotionProps(0.3)} className="w-[380px] flex flex-col">
            ...
            <div className="space-y-3 flex-1">
              {STEPS.map((step, i) => {
                ...
                return (
                  <motion.div
                    key={step.num}
                    {...getMotionProps(0.35 + i * 0.06)}
                    className={`flex items-center gap-3 p-3 ${colors.bg} border ${colors.border} rounded-xl backdrop-blur-sm`}
                  >
            ...
            {/* Note */}
            <motion.div {...getMotionProps(0.6)} className="mt-4 p-3 bg-white/[0.03] border border-white/10 rounded-xl">
              <p className="text-white/40 text-xs text-center">
                *La versión gratuita es suficiente para uso personal básico
              </p>
            </motion.div>
          </motion.div>
        </div>
