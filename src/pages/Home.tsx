...
import { Wrench, Workflow, BookOpen, Sparkles, Library, Users, ArrowRight, Lightbulb, GraduationCap, Target, Zap } from 'lucide-react';
            ...
            <div className="max-w-6xl mx-auto space-y-6">
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    Continua donde lo dejaste o explora algo nuevo
                  </p>
                </div>
                <kbd
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                  {[
                    ...
                    <Link
                      key={link.href}
                      to={link.href}
                      className="group flex flex-col items-center gap-2 p-3 rounded-xl glass hover:border-white/[0.1] transition-all duration-300 active:scale-[0.97]"
                    >
                      <div
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                {(() => {
                  ...
                  return (
                    <div className={`rounded-2xl p-4 border ${today.bg} flex items-start gap-3`}>
                      <today.icon className={`w-5 h-5 ${today.color} shrink-0 mt-0.5`} />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-semibold">{today.title}</h4>
                        <p className="text-xs text-muted-foreground mt-0.5">{today.text}</p>
                      </div>
                      <Link to={today.link} className="shrink-0 text-xs text-primary hover:text-primary/80 font-medium flex items-center gap-1 mt-1">
                        {today.linkText}
