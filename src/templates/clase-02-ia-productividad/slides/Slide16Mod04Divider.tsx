...
export function Slide16Mod04Divider() {
  ...
  return (
    <div className="slide-16-9 relative flex flex-col overflow-hidden bg-[#04030a] font-sans selection:bg-violet-500/30">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 50%, hsl(200 40% 8% / 0.5), transparent 70%)' }} />
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-16">
        <motion.span {...m(0)} className="text-sm font-bold tracking-[0.4em] text-sky-400/60 uppercase mb-6">
          04 /// MÓDULO 04
        </motion.span>
        <motion.h1 {...m(0.1)} className="text-6xl md:text-7xl font-serif text-white text-center mb-6">
          <EditableText defaultValue="El Paisaje de Modelos." tag="span" />
        </motion.h1>
        <motion.p {...m(0.2)} className="text-xl text-white/50 text-center max-w-2xl">
          <EditableText defaultValue="GPT-5.2 para estrategia, Perplexity para datos y DeepSeek para eficiencia." tag="span" />
        </motion.p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20">
        ...
        <div className="flex items-center justify-between px-12 py-4">
          ...
          <span className="text-[11px] font-bold tabular-nums tracking-wider text-white/60">{slideNum ? `${String(slideNum.current).padStart(2, '0')} / ${slideNum.total}` : '16 / 23'}</span>
        </div>
      </div>
