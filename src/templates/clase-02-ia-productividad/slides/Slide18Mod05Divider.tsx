...
export function Slide18Mod05Divider() {
  ...
  return (
    <div className="slide-16-9 relative flex flex-col overflow-hidden bg-[#04030a] font-sans selection:bg-violet-500/30">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 50%, hsl(340 40% 8% / 0.5), transparent 70%)' }} />
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-16">
        <motion.span {...m(0)} className="text-sm font-bold tracking-[0.4em] text-rose-400/60 uppercase mb-6">
          05 /// MÓDULO 05
        </motion.span>
        <motion.h1 {...m(0.1)} className="text-6xl md:text-7xl font-serif text-white text-center mb-6">
          <EditableText defaultValue="Agentes y Ejecución" tag="span" />
        </motion.h1>
        <motion.p {...m(0.2)} className="text-xl text-white/50 text-center max-w-2xl">
          <EditableText defaultValue="Anatomía agéntica, delegación con Manus, supervisión con Operator y el salto a director." tag="span" />
        </motion.p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20">
        ...
        <div className="flex items-center justify-between px-12 py-4">
          ...
          <span className="text-[11px] font-bold tabular-nums tracking-wider text-white/60">{slideNum ? `${String(slideNum.current).padStart(2, '0')} / ${slideNum.total}` : '18 / 23'}</span>
        </div>
      </div>
